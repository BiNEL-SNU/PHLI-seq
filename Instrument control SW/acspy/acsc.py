...
def setVelocity(hcomm, axis, vel, wait=SYNCHRONOUS):
    """Sets axis velocity"""
    return acs.acsc_SetVelocity(hcomm, axis, double(vel), wait)
...
def jog(hcomm, flags, axis, vel, wait=SYNCHRONOUS):
    """Jog move."""
    return acs.acsc_Jog(hcomm, flags, axis, double(vel), wait)

def toPoint(hcomm, flags, axis, target, wait=SYNCHRONOUS):
    """Point to point move."""
    return acs.acsc_ToPoint(hcomm, flags, axis, double(target), wait)
...
def enable(hcomm, axis, wait=SYNCHRONOUS):
    return acs.acsc_Enable(hcomm, int32(axis), wait)
...
def closeComm(hcomm):
    ...
    return acs.acsc_CloseComm(hcomm)
...
def getMotorError(hcomm, axis, wait=SYNCHRONOUS):
    error = ctypes.c_int()
    acs.acsc_GetMotorError(hcomm, axis, error, wait)
    return error.value
...
def waitMotionEnd(hcomm, axis, timeout= 10000):
    return acs.acsc_WaitMotionEnd(hcomm, axis,timeout)

def setFPosition(hcomm, axis, fposition):
    return acs.acsc_SetFPosition(hcomm, axis, ctypes.c_double(fposition), 0)
